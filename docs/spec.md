...
**添付ファイル情報(書類図形)**
- 親ノードのバウンディングボックスに重なる書類シェイプを検出し、`relatedFiles`(関連ファイル)に追加する
...
2. 矢印にテキストが無い場合、**分岐ノードの近くにあるYES/NOラベル図形**から判定する:
   ...
   - 候補図形のテキストが`YES`または`NO`と判定できるものを対象とする。判定時は英字以外の文字(`[`/`]`や前後の空白など装飾)を除去してから大文字小文字を区別せず比較する(例: `"[ No ]"` → `NO`、`"YES]"` → `YES`)
...
1. 左端列(A列)の実施主体名: 結合セルなし・非結合の場合の表現方法の確認
2. 書類図形とノードの重なり判定の具体的な閾値
3. 同じ位置で重なった書類図形同士の個別検出方法
4. 「DB」エリアが左端列の結合セル(実施主体名の一部)として表現されているか、別の見分け方があるか
...
> - YES/NOラベルは`"[ No ]"`や`"YES]"`のように装飾のかっこ・空白付きで書かれていることがある(4.2.3参照)
> - 分岐から手前のノードに戻る「やり直しループ」がある実データが存在する。分岐ルート判定はこれを正しく扱う(4.6.2参照)
> - 図形はExcelのグループ化機能(`xdr:grpSp`)でグループ化されていることがあり、素朴な実装だとグループ内の図形が丸ごと無視されてしまう
